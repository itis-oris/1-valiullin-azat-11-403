...
public class AuthenticationFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        // Public paths that don't require authentication - COMPLETELY SKIP AUTH
        if (isPublicPath(path)) {
            chain.doFilter(request, response);
            return; // IMPORTANT: return after public paths!
        }
    ...
    private boolean hasAccess(User user, String path) {
        ...
        // SERVICE paths - добавьте это правило!
        if (path.startsWith("/service/")) {
            // LABEL имеет доступ ко всем service путям
